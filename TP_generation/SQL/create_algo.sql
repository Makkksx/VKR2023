...
CREATE PROCEDURE Create_Algorithm  (
		...
		@n INT = 20,							
		@m INT = 10,
		@vr nvarchar(MAX) = '0.0, 0.3, 0.4, 0.3'
)
AS
...
BEGIN
	...
	IF NOT EXISTS (SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'Create_Algo_Result')
	BEGIN
		PRINT 'NOT EXISTS'
		Create Table Create_Algo_Result (Row_ INT, Col_ INT);
	END
	TRUNCATE TABLE Create_Algo_Result;


	INSERT INTO Create_Algo_Result 
...
vr_ = list(map(float,vr.split(", ")))
time_start = time.time()
gen = TPGeneration(n_cols=n, n_rows=m, vr=vr_)
print(gen.generate())
print("time", time.time() - time_start)
OutputDataSet = gen.pretty_df()
	'
	, @input_data_1 = @requestTable
	, @params = N'@resultCost INT OUTPUT, @n INT, @m INT, @vr nvarchar(MAX)'
